...
class ReminderReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        scope.launch {
            ...
            when (alarmType) {
                ALARM_MORNING -> {
                    // Only notify if the user is NOT already checked in
                    ...
                    // If already checked in, skip — no need to disturb them
                }

                ALARM_MIDDAY -> {
                    // Warn if user is not in office AND today's recorded hours are less than half the daily goal
                    val recordedSeconds = todayStat?.totalSeconds ?: 0L
                    val halfGoalSeconds = 3L * 3600 // 3 hours = half of 6h goal
                    if (activeSession == null && recordedSeconds < halfGoalSeconds) {
                        notificationHelper.sendNotification(
                ...
                ALARM_EVENING -> {
                    // Remind to check out if still active at end of day
                    if (activeSession != null) {
